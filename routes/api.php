...
Route::get('/update-blogger-chat', function () {
    try {
        ...
        if (str_contains($content, 'TELEGRAM_SUBMISSIONS_CHAT_ID=')) {
            $content = preg_replace('/TELEGRAM_SUBMISSIONS_CHAT_ID=[^\r\n]*/', 'TELEGRAM_SUBMISSIONS_CHAT_ID=-1004424306910', $content);
        } else {
            $content .= "\nTELEGRAM_SUBMISSIONS_CHAT_ID=-1004424306910\n";
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'TELEGRAM_SUBMISSIONS_CHAT_ID updated to -1004424306910 successfully!'
        ]);
    } catch (\Throwable $e) {
        return response()->json([
            'success' => false,
